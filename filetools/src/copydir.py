...
@version: 1.0.0
useage:
python D:\ssdftp\stability\deldir.py D:\ssdftp\stability\mu D:\ssdftp\iis\ftp1 D:\ssdftp\stability 3000 5
每隔3000秒拷贝删除D:\ssdftp\stability\mu目录5次到D:\ssdftp\iis\ftp1目录下，传输完成后，ftp1目录下文件数量为空时，删除空目录继续下轮拷贝，记录日志
到d:\ssdftp\stability\目录，日志文件名称为copydir_日期.log
'''
import os
import shutil
import sys
...
def writeLog(logdir,content):
    ...
    try:
        with open(logdir + os.path.sep + "copydir_" + time1 + ".log",'a') as f:
...
if __name__ == '__main__':
    ...
    logdir = sys.argv[3]   #日志文件存放路径，例如 "d:\ssdftp\stability"
...
#     logdir = r"d:\ftp\ftp1"
    i = 1
    writeLog(logdir, "开始拷贝" + srcdir +"到" + destdir +"目录")
    while i > 0:
        #拷贝文件
        print("开始拷贝") 
        r = copyDir(srcdir,destdir,i)
        if r:
            writeLog(logdir, destdir + os.path.sep + str(i))
        else:
            print("拷贝失败")
            sys.exit(1)
        if i % int(dirnum) == 0 :
            print("本轮拷贝完成")
            writeLog(logdir, "本轮拷贝完成")
            sleep(int(sleeptime))
            #判断目录是否为空,为空删除空文件夹继续下轮拷贝
            while not delEmpty(destdir):
                print("目录没有传输完成,等待20秒")
                sleep(30)
            writeLog(logdir, "删除空目录完成")
        i += 1